...
def list_in_db(orm_class, dataframe: pd.DataFrame, session: sessionmaker):
    ...
    for column in dataframe.columns:
        ...
        if column != 'x':
            ...
            entry = orm_class(id=i, name=column)
...
def get_deviations(df_from_x: pd.DataFrame, df_from_y: pd.DataFrame, estimators: list[LinearRegression]):
    ...
    for estimator in estimators:
        ...
        row['sqr_deviation'], row['max_deviation'] = calc_deviations(df_from_x, df_from_y, estimator)
...
def calc_deviations(x_data: pd.Series, y_data: pd.Series, function: LinearRegression):
    # calculates the square deviation and max deviation
...
def main():
    ...
    for column in df_train.columns:
        ...
        if column != 'x':
            ...
            # get all the deviations for current column and sort ascending by sqr_deviation
